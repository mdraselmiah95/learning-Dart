...
void main() {
  ...
  print(
    '$name, $city, $age, $gpa, $isStudent',
  ); // '$var' inserts value in string
  ...
  Set<int> unique = {
    ...
    3,
    3,
  }; // Set: unordered, NO duplicates → stores {1, 2, 3}
  ...
  print(
    val ?? 'default',
  ); // ?? = "use this if left side is null" → prints 'default'
  ...
  int score = 75;
  if (score >= 90) {
    // if this is true → run this block
    print('A');
  } else if (score >= 70) {
  ...
  print(
    greet(name: 'Rasel'),
  ); // named param: use label when calling → 'Hello, Rasel!'
  ...
  var doubled = nums
      .map((n) => n * 2)
      .toList(); // map runs function on each item
  ...
  print(
    maybeNull?.length ?? 0,
  ); // combine ?. with ?? for safe fallback → prints 0
  ...
  try {
    int bad = int.parse(
      'abc',
    ); // 'abc' can't become int → throws FormatException
  ...
  List<String> languages = ["Dart", "javascript", "Golang"];

  print('2nd Languages: ${languages[1]}');

  Map<String, int> studentMarks = {"Rasel": 90, "Miah": 95, "Ali": 85};

  print('Rasel Marks: ${studentMarks["Rasel"]}');
  print('Miah Marks: ${studentMarks["Miah"]}');
  print('Ali Marks: ${studentMarks["Ali"]}');

  dynamic anythingData = "This can be any type";
  anythingData = "Now it's an integer";
  anythingData = [1, 2, 3, 4, 5];

  print('Dynamic data: $anythingData');
...
Future<void> fetchData() async {
  await Future.delayed(
    Duration(seconds: 1),
  ); // wait 1 second (simulates API call)
...
class Person {
  ...
  Person(
    this.name,
    this.age,
  ); // constructor: 'this.name' auto-assigns param to field
